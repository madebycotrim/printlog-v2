...
export default function Logs() {
    ...
    const carregarLogs = async () => {
        try {
            ...
            const mockLogs = todosLogs.length > 0 ? todosLogs : Array.from({ length: 20 }).map((_, i) => ({
                id: i,
                action: i % 2 === 0 ? 'LOGIN_SUCCESS' : 'DATA_UPDATE',
                user_email: 'admin@escola.com',
                details: { ip: '192.168.1.1', changes: { field: 'name', old: 'A', new: 'B' } },
                timestamp: new Date().toISOString()
            }));

            mockLogs.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
            definirLogs(mockLogs);
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const logsFiltrados = logs.filter(l =>
        l.action?.toLowerCase().includes(busca.toLowerCase()) ||
        l.user_email?.toLowerCase().includes(busca.toLowerCase())
    );

    const totalPaginas = Math.ceil(logsFiltrados.length / LOGS_PER_PAGE);
    const logsPaginados = logsFiltrados.slice((pagina - 1) * LOGS_PER_PAGE, pagina * LOGS_PER_PAGE);

    const StatusBadge = ({ action }) => {
        let color = 'slate';
        if (action?.includes('SUCCESS') || action?.includes('CREATE')) color = 'emerald';
        if (action?.includes('ERROR') || action?.includes('DELETE')) color = 'rose';
        if (action?.includes('UPDATE') || action?.includes('WARNING')) color = 'amber';
        if (action?.includes('LOGIN')) color = 'blue';
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Detalhes</th>
                            </tr>
                        </thead>
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                ...
                                <p className="font-mono text-sm text-slate-700 font-bold">{logSelecionado.id}</p>
                            </div>
                            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                <p className="text-xs text-slate-400 uppercase font-bold mb-1">Timestamp</p>
                                <p className="font-mono text-sm text-slate-700 font-bold">{logSelecionado.timestamp}</p>
                            </div>
                        </div>
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-sm font-bold text-slate-700">Payload JSON</span>
                            </div>
