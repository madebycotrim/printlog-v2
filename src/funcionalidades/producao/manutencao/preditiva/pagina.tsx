import { Wrench, ArrowLeft, AlertTriangle, Calendar, Clock, CheckCircle2, MoreHorizontal, Settings2 } from "lucide-react";
import { useMemo } from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";

import { usarArmazemImpressoras } from "@/funcionalidades/producao/impressoras/estado/armazemImpressoras";
import { servicoPredicaoManutencao } from "@/compartilhado/infraestrutura/servicos/servicoPredicaoManutencao";
    ...
    return (
        <div className="space-y-10 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                >
                    <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${itensCriticos > 0 ? 'opacity-70' : 'text-gray-400'}`}>Intervenções Críticas</p>
                    ...
                    {itensCriticos > 0 && <p className="text-[10px] font-bold uppercase tracking-widest mt-4 bg-white/10 px-3 py-1 rounded-full w-fit italic">Ação Imediata Recomendada</p>}
                </motion.div>
                ...
                >
                    ...
                    <div className="h-2 w-full bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full bg-emerald-500" style={{ width: `${Math.round(((agenda.length - (itensCriticos + itensAviso)) / (agenda.length || 1)) * 100)}%` }} />
                    </div>
                </motion.div>
            </div>
            ...
            <div className="p-10 rounded-[2.5rem] bg-white dark:bg-[#121214] border border-gray-100 dark:border-white/5 shadow-sm">
                <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-zinc-500">Cronograma de Manutenção</h4>
                    </div>
                ...
                <div className="space-y-6">
                    {agenda.length === 0 ? (
                        <div className="py-20 text-center opacity-30">
                            ...
                            <p className="font-black uppercase tracking-widest text-xs">Tudo operacional. Nenhuma manutenção pendente.</p>
                        </div>
                    ) : (
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="md:col-span-1">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">{item.nomeImpressora}</p>
                                    ...
                                    <div className="md:col-span-1 space-y-3">
                                        <div className="flex justify-between items-end mb-1">
                                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${cores.bg} text-white`}>
                                                {item.status}
                                            </span>
            ...
            <div className="p-8 rounded-[2rem] bg-indigo-500/10 border border-indigo-500/20 flex items-center gap-6">
                ...
                <div>
                    <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-500 mb-1">Como calculamos a previsão?</h5>
                    <p className="text-xs font-medium text-indigo-700 dark:text-indigo-300 leading-relaxed">
                        Nossa IA analisa o horímetro atual e as horas de manutenção cadastradas. A projeção de data assume um uso médio de **8 horas diárias**. Para previsões mais precisas, mantenha o horímetro de suas máquinas sempre atualizado ao concluir cada projeto.
                    </p>
                </div>
            </div>
        </div>
