...
const MOCK_PECAS: PecaDesgaste[] = [
    { id: "1", idUsuario: "user", idImpressora: "dummy", nome: "Bico 0.4mm", minutosUsoAtual: 1200, vidaUtilEstimadaMinutos: 18000 },
    { id: "2", idUsuario: "user", idImpressora: "dummy", nome: "Correia X", minutosUsoAtual: 45000, vidaUtilEstimadaMinutos: 120000 },
    { id: "3", idUsuario: "user", idImpressora: "dummy", nome: "Lubrificação Eixos", minutosUsoAtual: 8000, vidaUtilEstimadaMinutos: 10000 },
];
...
class ServicoManutencao {
    async buscarManutencoes(idImpressora: string): Promise<Manutencao[]> {
        ...
        try {
            ...
            return MOCK_MANUTENCOES.filter((m) => m.idImpressora === idImpressora)
                .sort((a, b) => b.data.getTime() - a.data.getTime());
        } catch (erro) {
            registrar.error({ rastreioId, servico: "Manutenção", idImpressora }, "Falha ao buscar manutenções", erro);
            throw new ErroInterno("Erro ao carregar histórico de manutenção.", CodigoErro.ERRO_INTERNO, { idImpressora }, erro);
        }
    }
    ...
    async registrarManutencao(dados: RegistrarManutencaoInput): Promise<Manutencao> {
        ...
        try {
            ...
            registrar.info({ rastreioId, servico: "Manutenção", idManutencao: novaManutencao.id }, "Manutenção registrada com sucesso");
