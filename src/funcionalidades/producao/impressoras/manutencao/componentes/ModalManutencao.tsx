...
import { usarManutencao } from "../ganchos/usarManutencao";
            ...
            <div className="p-6 bg-white dark:bg-[#18181b]">
                ...
                <div className="flex gap-2 p-1 bg-secondary/30 rounded-xl mb-6">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black rounded-lg transition-all ${abaAtiva === "monitor" ? "bg-white dark:bg-zinc-800 shadow-sm border border-border" : "text-muted-foreground"
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black rounded-lg transition-all ${abaAtiva === "historico" ? "bg-white dark:bg-zinc-800 shadow-sm border border-border" : "text-muted-foreground"
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black rounded-lg transition-all ${abaAtiva === "novo" ? "bg-primaria text-white shadow-lg shadow-primaria/20" : "text-muted-foreground"
                            }`}
                    >
                                ...
                                manutencoes.map(m => (
                                    <div key={m.id} className="p-4 rounded-xl border border-border bg-card/50">
                                        <div className="flex justify-between items-start mb-2">
                                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${m.tipo === "Preventiva" ? "border-emerald-500/30 text-emerald-500 bg-emerald-500/5" : "border-amber-500/30 text-amber-500 bg-amber-500/5"
                                                }`}>
                                                {m.tipo}
                                            </span>
                    ...
                    {abaAtiva === "novo" && (
                        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}>
                            <FormularioManutencao
                                ...
                                aoSalvar={async (dados) => { await registrarManutencao(dados); }}
