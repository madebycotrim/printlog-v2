...
export function PaginaHistoricoProducao() {
    ...
    const exportarHistorico = () => {
        const dadosExport = pedidos.filter(p => p.status === "concluido").map(p => ({
            id: p.id,
            cliente: p.idCliente,
        ...
        <div className="space-y-10 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                >
                    ...
                    <h3 className="text-3xl font-black text-zinc-900 dark:text-white">{Math.round(historico.totalTempoMinutos / 60)}h</h3>
                ...
                >
                    ...
                    <h3 className="text-3xl font-black text-zinc-900 dark:text-white">{centavosParaReais(historico.totalFaturadoCentavos)}</h3>
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                ...
                                <p className="text-4xl font-black text-emerald-400">
                                    {centavosParaReais(historico.totalProjetosConcluidos > 0
            ...
            <div className="p-10 rounded-[2.5rem] bg-white dark:bg-[#121214] border border-gray-100 dark:border-white/5 shadow-sm">
                <h4 className="text-xs font-black uppercase tracking-widest text-gray-400 mb-10">Projetos Recentemente Concluídos</h4>
                <div className="space-y-4">
                    {pedidos.filter(p => p.status === "concluido").slice(-5).reverse().map((p) => (
                        <div key={p.id} className="flex items-center justify-between p-6 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-transparent hover:border-sky-500/20 transition-all group">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1">Concluído em: {p.dataConclusao?.toLocaleDateString('pt-BR') || "---"}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-10">
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-gray-400 uppercase mb-1">Recursos</p>
                                    <p className="text-xs font-black">{p.pesoGramas}g • {p.tempoMinutos}min</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-gray-400 uppercase mb-1">Valor</p>
                                    <p className="text-sm font-black text-zinc-900 dark:text-white">{centavosParaReais(p.valorCentavos)}</p>
                                </div>
                            </div>
                        </div>
                    ))}
