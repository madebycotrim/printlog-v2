...
interface PropriedadesFiltrosFinanceiro {
    tipoAtivo: string | null;
    aoMudarTipo: (tipo: string | null) => void;
    ordenacaoAtual: OrdenacaoFinanceiro;
...
export function FiltrosFinanceiro({
    ...
    const opcoesOrdenacao: { valor: OrdenacaoFinanceiro; rotulo: string }[] = [
        { valor: "DATA", rotulo: "Última Atualização" },
        { valor: "VALOR", rotulo: "Maior Valor" },
        { valor: "DESCRICAO", rotulo: "Ordem Alfabética" },
    ];
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between mb-8">
            ...
            {/* Filtros de Tipo (Padronizado e Neutro: TODOS, ENTRADA, SAÍDA) */}
            <div className="flex items-center p-1 bg-zinc-100/50 dark:bg-white/[0.03] border border-zinc-200/50 dark:border-white/5 rounded-full overflow-x-auto no-scrollbar max-w-full md:max-w-2xl">
                <button
                    onClick={() => aoMudarTipo(null)}
                    className={`px-5 py-2 text-xs font-bold rounded-full transition-all whitespace-nowrap ${tipoAtivo === null
                ...
                <button
                    onClick={() => aoMudarTipo('entrada')}
                    className={`px-5 py-2 text-xs font-bold rounded-full transition-all whitespace-nowrap ${tipoAtivo === 'entrada'
                        ? "bg-zinc-800 dark:bg-white text-white dark:text-zinc-950 shadow-sm"
                        : "text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-300"
                        }`}
                >
                ...
                <button
                    onClick={() => aoMudarTipo('saida')}
                    className={`px-5 py-2 text-xs font-bold rounded-full transition-all whitespace-nowrap ${tipoAtivo === 'saida'
                        ? "bg-zinc-800 dark:bg-white text-white dark:text-zinc-950 shadow-sm"
                        : "text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-300"
                        }`}
                >
            ...
            <div className="flex items-center gap-2 w-full md:w-auto self-end md:self-center">
                ...
                <div className="relative" ref={referenciaSeletor}>
                    <button
                        ...
                        className={`h-11 px-4 text-sm font-medium bg-transparent border-b-2 flex items-center justify-between gap-3 min-w-[200px] transition-all outline-none ${seletorAberto
