...
export function CardAparencia({ pendente }: CardAparenciaProps) {
    const { modo_tema, alternar_tema, cor_primaria, definir_cor_primaria, fonte, definir_fonte } = usarContextoTema();

    return (
        <div className="rounded-3xl border border-gray-200 dark:border-white/10 bg-white dark:bg-card-fundo p-4 md:p-5 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-2">
                {[
                    ...
                    <button
                        ...
                        className={`rounded-xl border p-2.5 flex items-center gap-2 transition-all outline-none ${modo_tema === modo.id
                    ...
                    >
                        ...
                        {modo_tema === modo.id && <Check size={12} className="text-emerald-500" />}
                    </button>
                ))}
            </div>
                ...
                <div className="flex flex-wrap justify-center gap-1.5">
                    {coresDisponiveis.map((cor) => (
                        <button
                            ...
                            className={`relative w-7 h-7 rounded-full ${cor.class} border-[3px] border-white dark:border-card-fundo transition-all hover:scale-110 active:scale-95 shadow-sm ${cor_primaria === cor.id ? "outline outline-offset-1 scale-110" : ""}`}
                            style={cor_primaria === cor.id ? { outlineColor: "var(--cor-primaria)", outlineWidth: "2px" } : undefined}
                            aria-label={`Selecionar Tema ${cor.nome}`}
                        >
                            {cor_primaria === cor.id && <Check size={10} className="absolute inset-0 m-auto text-white drop-shadow-md" />}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1.5 flex-1">
                    {fontesDisponiveis.map((f) => (
                        <button
                            key={f.id}
                            onClick={() => definir_fonte(f.id as any)}
