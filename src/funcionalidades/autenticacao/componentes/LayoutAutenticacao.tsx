    ...
    return (
        <div className="min-h-screen w-full font-sans bg-[#050505] relative flex items-center justify-center p-4 overflow-hidden selection:bg-[#0ea5e9] selection:text-white">
            ...
            <nav className="absolute top-6 left-6 z-50">
                <a href={linkVoltar} className="group flex items-center gap-2 text-zinc-500 hover:text-white transition-colors text-sm font-medium">
            ...
            {/* Card Principal */}
            <div className={`w-full ${larguraMaxima} min-h-[450px] bg-zinc-900/60 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl flex overflow-hidden relative z-10 animate-fade-in-up`}>
                {children}
            </div>
        </div>
    );
