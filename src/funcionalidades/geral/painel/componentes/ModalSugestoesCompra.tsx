...
import { SugestaoCompra } from "@/compartilhado/infraestrutura/servicos/servicoInventario";
...
export function ModalSugestoesCompra({ aberto, aoFechar, sugestoes }: ModalSugestoesCompraProps) {
    ...
    return (
        <Dialogo
            aberto={aberto}
            aoFechar={aoFechar}
            titulo="Análise Preditiva de Compras"
        ...
        >
            <div className="p-8 space-y-8 bg-white dark:bg-[#121214]">
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-sky-500/5 border border-sky-500/10">
                    ...
                    <div>
                        <h4 className="text-xs font-black uppercase tracking-widest text-sky-600 dark:text-sky-400 mb-1">Cesta Sugerida</h4>
                        <p className="text-[11px] font-medium text-gray-500 dark:text-gray-400 leading-relaxed">
                            Baseado no seu consumo dos últimos 30 dias, estas aquisições são recomendadas para manter seu estúdio operando sem interrupções.
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    {sugestoes.length === 0 ? (
                        <div className="py-20 text-center opacity-30">
                            <p className="text-sm font-black uppercase tracking-widest">Estoque saudável. Nenhuma compra necessária.</p>
                        </div>
                    ) : (
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2 h-2 rounded-full ${s.prioridade === "Crítica" ? "bg-rose-500 animate-pulse" : s.prioridade === "Alta" ? "bg-amber-500" : "bg-sky-500"}`} />
                                            <h5 className="text-sm font-black uppercase tracking-tight">{s.nome}</h5>
                                        </div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{s.tipo} • {s.quantidadeAtual}{s.unidade} em estoque</p>
                                    </div>
                                    <div className="text-right">
                                        <div className={`text-[9px] font-black uppercase px-2 py-1 rounded-md mb-2 inline-block ${s.prioridade === "Crítica" ? "bg-rose-500 text-white" :
                                            s.prioridade === "Alta" ? "bg-amber-500 text-white" :
                                                "bg-sky-500/10 text-sky-500"
                                            }`}>
                                            Prioridade {s.prioridade}
                                        </div>
                                        <p className="text-xs font-black text-gray-900 dark:text-white">Faltam aprox. {s.diasRestantes} dias</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-white/5">
                                    <div className="flex items-center gap-4">
                                        <div className="space-y-0.5">
                                            <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest underline decoration-sky-500/30">Sugestão de Compra</p>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-sm font-black text-gray-900 dark:text-white">{centavosParaReais(s.custoEstimadoCentavos)}</p>
                                    </div>
                                </div>
                            </motion.div>
                        ))
                ...
                {sugestoes.length > 0 && (
                    <div className="pt-6 border-t border-gray-100 dark:border-white/5 flex flex-col items-center gap-6">
                        <div className="flex justify-between w-full items-center">
                            <div className="space-y-1">
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Total do Pedido Sugerido</p>
                                <p className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter">{centavosParaReais(custoTotal)}</p>
                            </div>
