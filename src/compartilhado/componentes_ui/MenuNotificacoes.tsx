...
import { TipoNotificacao } from "../tipos_globais/notificacoes";

export function MenuNotificacoes() {
    const { notificacoes, marcarComoLida, marcarTodasComoLidas, limparNotificacoes, removerNotificacao } = usarArmazemNotificacoes();
    ...
    const obterIconeTipo = (tipo: TipoNotificacao) => {
        switch (tipo) {
            case TipoNotificacao.SUCESSO: return <CheckCircle2 className="text-emerald-500" size={18} />;
            case TipoNotificacao.AVISO: return <AlertTriangle className="text-amber-500" size={18} />;
            case TipoNotificacao.ERRO:
            case TipoNotificacao.CRITICO: return <AlertCircle className="text-rose-500" size={18} />;
                    ...
                    >
                        ...
                        <div className="p-5 border-b border-gray-100 dark:border-white/5 flex items-center justify-between bg-gray-50/50 dark:bg-white/2">
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Alertas e Atividades</p>
                            </div>
