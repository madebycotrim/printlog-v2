...
export const Carretel = memo(
  ({
    ...
    const porcentagemSegura = Math.max(
      0,
      Math.min(100, Number(porcentagem) || 0),
    );
    ...
    const raioAtualY =
      raioLeitoY + (maxFilamentoY - raioLeitoY) * taxaPreenchimento;
    const raioAtualX =
      raioLeitoX + (maxFilamentoX - raioLeitoX) * taxaPreenchimento;
...
export const GarrafaResina = memo(
  ({
    ...
    const idUnico = (id || useId()).replace(/:/g, "");
    const { base, claro, escuro } = useMemo(
      () => gerarCores(cor || "#3b82f6"),
      [cor],
    );
    ...
    return (
      ...
      >
        <svg
          viewBox="0 0 100 100"
          className="relative w-full h-full overflow-visible drop-shadow-lg"
        >
          {/* Tampa Cartoon */}
          ...
          <use
            href={`#bottlePath-${idUnico}`}
            fill={corPlasticoGarrafa}
            opacity={0.6}
          />{" "}
