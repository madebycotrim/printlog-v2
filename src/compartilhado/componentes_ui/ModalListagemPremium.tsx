    ...
    return (
        ...
        >
            <div className="flex flex-col h-[85vh] bg-white dark:bg-[#18181b]">
                ...
                <div className={`px-8 py-6 border-b border-gray-100 dark:border-white/5 bg-${corDestaque}-50/30 dark:bg-${corDestaque}-500/[0.02] backdrop-blur-sm sticky top-0 z-20`}>
                    <div className="relative group">
                        <Search className={`absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-${corHex} transition-colors`} size={18} />
                        <input
                            type="text"
                            value={termoBusca}
                ...
                <div className="flex-1 overflow-y-auto p-8 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/5">
                    {!temResultados && !loading ? (
                        <EstadoVazio
                            ...
                            descricao={mensagemVazio}
                            icone={iconeVazio || Search}
                        />
                    ) : (
                        children
                    )}
                ...
                {/* 📑 Rodapé Informativo */}
                <div className={`px-8 py-4 border-t border-gray-100 dark:border-white/5 bg-${corDestaque}-50/20 dark:bg-${corDestaque}-500/[0.01] flex items-center justify-between`}>
