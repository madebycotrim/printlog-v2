...
export function CardResumo({
    ...
    return (
        ...
        >
            ...
            {aoClicar && (
                <div className={`absolute top-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 ${estilo.texto}`}>
            ...
            {/* Ícone */}
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center border relative z-10 transition-transform group-hover:scale-110 duration-300 ${estilo.bg} ${estilo.texto}`}>
            ...
            <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-1.5 mt-0.5">
                    <span className={`text-2xl font-black tracking-tight ${carregando ? "animate-pulse bg-zinc-200 dark:bg-zinc-800 rounded h-8 w-12" : "text-gray-900 dark:text-white"}`}>
                        {!carregando && valor}
                    </span>
                    {unidade && !carregando && (
 ...
 */
export function CardResumoVazio({ texto = "MAIS INDICADORES EM BREVE", icone: Icone }: { texto?: string, icone: ElementType }) {
    return (
