...
export const servicoManutencao = {
     ...
     */
    registrarManutencaoConcluida: (idImpressora: string, dados: Omit<RegistroManutencao, 'id' | 'horasMaquinaNoMomentoMinutos'>) => {
        const { impressoras, definirImpressoras } = usarArmazemImpressoras.getState();
        const novas = impressoras.map(i => {
            if (i.id === idImpressora) {
        ...
        definirImpressoras(novas);
        registrar.info({ rastreioId: idImpressora, tipo: dados.tipo, servico: "Manutenção" }, "Manutenção registrada com sucesso");
    },
    ...
    preverProximaManutencao: (impressora: Impressora): Date | null => {
        ...
        const ultimaData = new Date(impressora.historicoProducao[impressora.historicoProducao.length - 1].dataConclusao).getTime();
